...
class IntradayStrategyMinuteCacheTest(unittest.TestCase):
    ...
    def test_morning_run_refreshes_stale_cached_minute_data(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            fresh_df = _minute_frame(
                [
                    ...
                    {
                        "trade_time": "2026-04-17 10:19:00",
                        "price": 10.4,
                        "change": 0.4,
                        "change_pct": 4.0,
                        "volume": 180,
            ...
            self.assertListEqual(result["time_only"].tolist(), ["09:30:00", "09:31:00", "10:19:00"])
            mocked_fetch.assert_called_once()

    def test_morning_run_keeps_fresh_cached_minute_data(self):
        strategy = IntradaySignalStrategy(candidate_size=1, minute_cache_ttl_seconds=120)
        with tempfile.TemporaryDirectory() as tmpdir:
            strategy.minute_cache_dir = os.path.join(tmpdir, "minute_live")
            cached_df = _minute_frame(
                [
                    {
                        "trade_time": "2026-04-17 10:18:30",
                        "price": 10.1,
                        "change": 0.1,
                        "change_pct": 1.1,
                        "volume": 120,
            ...
            with patch.object(
                ...
                result = strategy.fetch_minute_data("000001", trade_date="2026-04-17", prefer_cache=True)

            self.assertEqual(len(result), 1)
            self.assertListEqual(result["time_only"].tolist(), ["10:18:30"])
